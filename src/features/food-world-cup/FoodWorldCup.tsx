...
export default function FoodWorldCup({
  ...
  return (
    ...
    >
      ...
      <h2 className="text-4xl font-bold text-white mb-8">{getRoundTitle()}</h2>

      <AnimatePresence mode="wait">
        {finalWinner ? (
          // 최종 우승 UI
          <motion.div key="winner" className="flex flex-col items-center">
            <FaTrophy className="text-yellow-400 text-7xl mb-4" />
          ...
          currentMatchup && (
            ...
            >
              ...
              <motion.div
                ...
                className="w-1/2 p-4 flex flex-col items-center cursor-pointer group"
                whileHover={{ scale: 1.05 }}
              >
                <div className="relative w-full h-80">
                  <WorldCupImage food={currentMatchup[0]} />
                </div>
                <h3 className="text-2xl font-bold text-white mt-4 group-hover:text-yellow-400">
                  {currentMatchup[0].name}
                </h3>
              </motion.div>
              <span className="text-5xl font-bold text-white mx-8">VS</span>
              {/* 오른쪽 선택지 */}
              <motion.div
